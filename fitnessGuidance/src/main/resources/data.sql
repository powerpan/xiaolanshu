...
(6, '训练打卡不是签到，而是复盘数据', '打卡的价值不在于留下一个完成标记，而在于沉淀趋势。训练时长、状态、备注和连续天数能帮助你判断计划是否过轻、过重或刚好合适。若连续打卡但状态越来越差，说明需要恢复；若很少打卡但每次时长很长，说明计划可能不够稳定。好的系统应该帮助用户降低记录成本，并把记录转化为下一步建议。', 'admin', '习惯养成', DATEADD('DAY', -5, CURRENT_TIMESTAMP));
